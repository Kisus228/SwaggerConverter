...
using Kontur.Elba.Web.Infrastructure.Common.Helpers;
using Kontur.Elba.Web.Infrastructure.Common.Mvc.ActionResults;
using Kontur.Elba.Xamarin.DataContracts.CrossplatformTools.EnumDescription;
...
namespace Kontur.Elba.SwaggerDocumentationGenerator.Helpers
{
	internal static class TypeHelpers
	{
		public static IEnumerable<Type> GetReturnValues(MethodInfo methodInfo)
		{
			...
			var apiResultTypes = methodInfo.UnwrapApiResultTypes();
			if (apiResultTypes.HasValue)
				foreach (var type in apiResultTypes.Value)
					yield return type;
			else
		...
		{
			var fileResultTypes = new []{typeof(FileResult), typeof(BasicFile), typeof(System.Web.Mvc.FileResult)};
		...
		public static bool HasNoReturnValue(this MethodInfo methodInfo)
		{
